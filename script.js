...
let numOfpeople = 20 ; // number of people in city 
...
function moveObjects() {  // here we change the x-coordinate and y-coordinate randomly but the balls while get out of the canvase 
...
function ballCollision() { // here tha main function for collision 
    for (let i=0; i<ballArray.length; i++) {
        for (let j=i+1; j<ballArray.length; j++) {
            ...
            if (dist < ob1.radius + ob2.radius) {  // here when the balls are collision  
    ...
    if (ballArray.length > 0)
        wallCollision(ballArray[ballArray.length-1])
}
...
for (var i = 0; i < numOfpeople ; i++) {
	if(i == 0 ){ballArray[i] = new Ball (xCoordinate(),yCoordinate(),raduius() , 0);}
	else
	ballArray[i] = new Ball (xCoordinate(),yCoordinate(),raduius() , 1);
